...
def calculate_brenner(img):
    ...
    # 计算 Brenner 清晰度
    score = np.sum(np.square(np.diff(img)) / (img.shape[0] * (img.shape[1] - 1)))
    return score


sharpnessMethod = [
    calculate_laplacian,
    calculate_tenerngrad,
    calculate_brenner,
]


def getLineSharpness(currentScreen, lineRegionTopLeft, lineRegionBottomRight, method_index=0):
    try:
        if lineRegionTopLeft == 0:
            ...
            sharpness = round(sharpnessMethod[method_index](grayRegion), 2)
            # sharpness = sharpness
...
for i in range(1, 75):
    ...
    sharpness = getLineSharpness(currentScreen, topLeft, bottomRight,2)
    print(f"sharpness of {i}.png is that: {sharpness}")
